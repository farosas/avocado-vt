...
class VirtIface(propcan.PropCan, object):
    ...
    @classmethod
    def generate_hex(cls, digits):
        """
        Return next hexadecimal number from ring as a string
        """
        cls.LASTBYTE += 1
        if cls.LASTBYTE > pow(16, digits) - 1:
            cls.LASTBYTE = 0
        return hex(cls.LASTBYTE)[2:]

    @classmethod
    def complete_mac_address(cls, mac_prefix):
        """
        Append a sequential MAC address to mac_prefix

        :param mac_prefix: str or list of mac bytes
        :raise: TypeError if mac_prefix is not a valid MAC address
        part
        """
        mac_prefix = cls.mac_str_to_int_list(mac_prefix)
        prefix_octets = len(mac_prefix)

        if prefix_octets == 6:
            return ":".join(cls.int_list_to_mac_str(mac_prefix))

        suffix_digits = (6 - prefix_octets) * 2
        hex_str = cls.generate_hex(suffix_digits).rjust(suffix_digits, '0')
        mac_suffix = ':'.join(hex_str[i:i+2]
                              for i in range(0, len(hex_str), 2))

        return ":".join(cls.int_list_to_mac_str(mac_prefix)) + ":" + mac_suffix
